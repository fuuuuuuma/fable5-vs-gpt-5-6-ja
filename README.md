...
9. [実際の出力を並べてみる（再現イメージ）](#9-実際の出力を並べてみる再現イメージ)
...
## 9. 実際の出力を並べてみる（再現イメージ）

数字だけだと伝わりにくいので、**同じ質問を両方に投げたときの出力の“らしさ”**を並べてみます。以下は実際の応答の傾向を踏まえて作った**再現イメージ**で、内容そのものは説明用に整えたものです（実際の出力は毎回変わります）。スライド版では、この見比べを大きな画面で見られます。

**お題:「初心者向けに、料理教室のホームページ（LP）の構成案を作って」**

**― Fable 5 の“らしさ”（深く・確認を挟む・盲点を突く）**

```text
構成案を出す前に、仕上がりを左右する点を3つ確認させてください。
①集客のゴールは「体験予約」か「メール登録」か。導線が変わります。
②ターゲットは初心者主婦層か、若い単身者か。写真と言葉選びが変わります。
③既に強み（少人数制・駅近・食材へのこだわり等）はありますか。

（前提を仮置きした上で）構成は次の7ブロックを推奨します。
1. ファーストビュー：「◯◯が作れるようになる」変化を1行で／体験予約ボタン
2. 共感：料理の悩み3つ ... 
※ よくある失敗：教室の「特徴」から書き始めてしまうこと。
  読者は特徴より「自分がどう変われるか」を先に知りたい。ここを逆にしないこと。
```

**― GPT-5.6 Sol の“らしさ”（速い・簡潔・すぐ使える手順）**

```text
料理教室LPの構成案です。上から順に置けば形になります。

1. ヒーロー：キャッチコピー＋体験予約ボタン＋教室写真
2. 悩み共感：「レシピ通りでも味が決まらない」等3つ
3. 選ばれる理由：少人数/初心者歓迎/駅近 の3枚カード
4. コース紹介：月謝・回数・持ち物を表で
5. 生徒の声：写真＋一言を3件
6. 講師紹介：顔写真＋経歴
7. よくある質問→最後にもう一度、予約ボタン

このまま画像も10枚生成しますか？
```

見比べると性格がよく出ます。**Fable 5 は「作る前に、あなたが決めきれていないこと（未知）を潰しにくる」**。確認質問と「よくある失敗」の指摘が入り、精度は高いぶん、ひと手間かかります。**GPT-5.6 は「とりあえず形になる完成品を、速く、そのまま渡してくる」**。次の一手（画像生成）まで提案して、勢いよく前に進めます。

**どちらが“良い”かは場面次第**です。じっくり練りたい企画の入り口なら Fable 5 の問いかけが効くし、今すぐたたき台が欲しいなら GPT-5.6 の速さが効く。**同じお題を両方に投げて、この違いを自分の目で見比べる**のが、相棒選びのいちばんの近道です（そのための比較用プロンプトを100本、後半のプレゼントにまとめました）。
